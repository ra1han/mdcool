...
export async function renderMarkdown(mdText, opts) {
...
function renderFrontmatterMeta(fm) {
    const items = [];
...
function renderGitHubAlerts(html) {
    // Convert GitHub-style alerts: <blockquote><p>[!NOTE]\nContent</p></blockquote>
    return html.replace(/<blockquote>\s*<p>\[!(NOTE|TIP|IMPORTANT|WARNING|CAUTION)\]\s*\n?([\s\S]*?)<\/p>\s*<\/blockquote>/gi, (_match, type, content) => {
...
function stripFrontmatter(mdText) {
    ...
    if (!match)
        return { content: mdText, frontmatter: null };
...
function wrapSectionsCollapsible(html) {
    ...
    for (let i = 0; i < matches.length; i++) {
        ...
        parts.push(`<details class="collapsible-section"${idAttr}>\n` +
